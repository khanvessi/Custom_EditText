...
@InverseBindingMethods(value = [InverseBindingMethod(type = CustomEditText::class, attribute = "textValue", event = "android:textAttrChanged", method = "getTextValue"),
    InverseBindingMethod(type = CustomEditText::class, attribute = "errorTextValue"),
    InverseBindingMethod(type = CustomEditText::class, attribute = "isErrorEnable")])
class CustomEditText @JvmOverloads constructor(context: Context, attrs: AttributeSet?, defStyle: Int = 0, defStyleRes: Int = 0) : LinearLayout(context, attrs, defStyle, defStyleRes) {
    ...
    init {
        LayoutInflater.from(context).inflate(R.layout.custom_editext, this, true)
        ...
        attrs?.let {
            ...
            val isMandatory = typedArray.getBoolean(R.styleable.custom_component_attributes_custom_component_is_mandatory, false)
            ...
            val titleBgColor = typedArray.getColor(R.styleable.custom_component_attributes_custom_component_title_bg_color, ContextCompat.getColor(context, R.color.design_default_color_on_primary))
            val editTextBgColor = typedArray.getColor(R.styleable.custom_component_attributes_custom_component_editText_bg_color, ContextCompat.getColor(context, R.color.design_default_color_on_primary))
            val errorTextBgColor = typedArray.getColor(R.styleable.custom_component_attributes_custom_component_error_text_bg_color, ContextCompat.getColor(context, R.color.design_default_color_on_primary))
    ...
    fun setIsMandatory(isMand: Boolean){
        if(isMand){
            mTitle = lableTitle.text as String?
            lableTitle.text = "$mTitle (Mandatory)"
        }else{
            lableTitle.text = lableTitle.text.toString() + " (Optional)"
        }
    }
